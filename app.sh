p6_aws_ec2_svc_instance_create() {
    ...
    [ -z "$ami_id" ]    && ami_id=$(p6_aws_ec2_svc_amis_freebsd12_latest)
    [ -n "$user_data" ] && user_data="--user-data=$user_data"

    local key_name=$(p6_aws_ec2_svc_key_pair_make "$USER")

    local sg_id
    local subnet_id
    case $name in
	bastion)
	    sg_id=$(p6_aws_ec2_svc_sg_id_from_group_name "bastion-ssh")
	    subnet_id=$(p6_aws_ec2_svc_subnet_bastion_get)
	    ;;
	*)
	    sg_id=$(p6_aws_ec2_svc_sg_id_from_group_name "vpc-ssh")
	    subnet_id=$(p6_aws_ec2_svc_subnet_${subnet_type}_get)
	    ;;
    esac

    local sg_outbound_id=$(p6_aws_ec2_svc_sg_id_from_group_name "outbound")
...
p6_aws_ec2_svc_instance_ubuntu18_create() {

    p6_aws_ec2_svc_instance_create "ubuntu18"
}

p6_aws_ec2_svc_instance_rhel8_create() {

    p6_aws_ec2_svc_instance_create "rhel-8"
}

p6_aws_ec2_svc_instance_amazon_create() {

    p6_aws_ec2_svc_instance_create "amazon2"
}

p6_aws_ec2_svc_cleanup() {
...
p6_aws_ec2_svc_sg_bastion_ssh_create() {
...
p6_aws_ec2_svc_sg_instance_ssh_create() {
    local vpc_id=${1:-$AWS_VPC}
...
p6_aws_ec2_svc_sg_outbound_ssh_create() {
    local vpc_id=${1:-$AWS_VPC}
...
p6_aws_ec2_svc_sg_link_outbound_world_world() {
...
p6_aws_ec2_svc_sg_link_bastion_world_ssh() {

    local sg_bastion_ssh_id=$(p6_aws_ec2_svc_sg_id_from_group_name "bastion-ssh")
    p6_aws_ec2_security_group_ingress_authorize --group-id $sg_bastion_ssh_id --protocol tcp --port 22 --cidr 0.0.0.0/0
}

## Links: a_b_proto
p6_aws_ec2_svc_sg_link_bastion_vpc_ssh() {

    local sg_bastion_ssh_id=$(p6_aws_ec2_svc_sg_id_from_group_name "bastion-ssh")
    local sg_instance_ssh_id=$(p6_aws_ec2_svc_sg_id_from_group_name "vpc-ssh")
...
p6_aws_ec2_svc_instance_bastion_create() {

    p6_aws_ec2_svc_instance_create "bastion"
}

p6_aws_ec2_svc_instance_irc_create() {

    local ami_id=$(p6_aws_ec2_svc_amis_freebsd12_latest)
    p6_aws_ec2_svc_instance_create "irc" "$ami_id" "t2.micro"
}

p6_aws_ec2_svc_instance_jenkins_create() {

    local ami_id=$(p6_aws_ec2_svc_amis_freebsd12_latest)
    p6_aws_ec2_svc_instance_create "jenkins" "$ami_id" "m4.large"
}
